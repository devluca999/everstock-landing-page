...
**Hero placement + synced beam flourish (HERO-ONLY — standing decision).** The marketing hero carries a second instance of the mark (display cut), positioned below the EVERSTOCK title and above the tagline. Its position is **deliberately decoupled** from the title's live font/effect state: the title lives in a fixed-height clearance zone sized for the worst-case box (tallest font, e.g. Anton × 1.16, plus effect-layer transform overflow), so the logo sits a fixed distance below and **never shifts** whichever font/effect is active. Do not re-tie the logo's position to the title's rendered size in a future pass — the fixed clearance is intentional. Layered on top of the (unchanged, pure segmented-chrome) hero mark is a **beam flourish synced to the hero title's rare/signature effect layer**: when Fuzzy/Particle fires (the ~10% class), a thin beam (blue, ~40% green — same ratio as the site beam system) travels once along the mark's ring/facet seams (following the curve, not cutting across the metal), then fades to the static resting state. `prefers-reduced-motion` → a static glow tick instead of the traveling beam. **This flourish is hero-only.** The **nav and favicon** instances of the mark are **always fully static — never animated, no synced beam** — do not add the flourish to them.
