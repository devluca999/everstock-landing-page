...
- **Hero floor (new, `components/HeroFloor.tsx` + `lib/heroFloor.ts`).** A blurred perspective conveyor drawn at 1/3 resolution (1/4 on phones) under the lattice: two-point ground-plane camera (26° yaw, horizontal horizon at 24% height, every vertical drawn straight up the screen, no rotation transforms), cross-rollers even in `t`, crates in loose clusters, a scanner gate whose light is **always blue**, crates that pick up a transient green (pass, ~1s) or deep-orange (deny, ~2s) glow that dissipates to exactly zero, and an articulated pick-and-place arm that eases the belt to a stop and lifts the denied crate out of frame on a jittered ~7s cadence. Layer order: floor → `.v2-solid` (opaque graphite across the left third, gone by the right edge) → `.v2-scrim` (radial legibility scrim under the wordmark) → lattice. `GridBackground` now draws **transparent** frames and cuts a soft hole over the deck footprint (`deckFootprint`) so the lattice and beams sit back over the conveyor without vanishing. ~30fps cap (20 on phones), paused when hidden or scrolled past 1.15vh, one static frame under reduced motion. The warm/steel floor palette never touches UI chrome; light theme only dims its emissive passes. **Performance rules learned the hard way (2026-09-14):** never put a CSS `mask-image`, `filter` or `mix-blend-mode` on the hero canvases or a root-level hero layer; any of them takes the canvas off the compositor's fast path and turns every animation frame into a long main-thread task. Every fade, cutout and soft pass is baked into the small canvas buffers instead (the lattice's lower-edge fade + conveyor cutout are one quarter-res `destination-in` blit per frame; the floor's static set is cached per resize and its blur is a two-step downsample + upscale, not a canvas filter; the sharp pass takes a half-res round trip for anti-aliasing). Beam-head glows are pre-rendered sprites; the lattice runs at 2× pixel density on desktop and 1.5× under 1024px (1.5× on desktop read visibly soft on retina, so **do not drop density to buy performance**: the pixels were never the cost, the CSS masks were); the hologram clips are `preload="none"` until their section approaches. Production Lighthouse after the pass: desktop 100, mobile median 91 across five runs (mobile blocking time is noisy run to run).
